<?php
declare(strict_types=1);
namespace Snappbox;

if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

class SnappBoxScheduleModal {
	private $allowed_days = array( 'saturday', 'sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday' );
	public function __construct() {
		\add_action( 'admin_enqueue_scripts', array( $this, 'snappb_enqueue_assets' ) );
		\add_action( 'wp_ajax_snappb_save_schedule', array( $this, 'snappb_save_schedule' ) );
	}

	public function snappb_enqueue_assets() {
		\wp_enqueue_script(
			'SnappBoxData',
			SNAPPBOX_URL . 'assets/js/snappbox-schedule-script.js',
			array( 'jquery' ),
			'1.0.1',
			true
		);

		\wp_localize_script(
			'SnappBoxData',
			'SnappBoxData',
			array(
				'ajax_url' => \admin_url( 'admin-ajax.php' ),
				'nonce'    => \wp_create_nonce( 'snappbox_schedule_nonce' ),
			)
		);
	}

	public function snappb_render_modal_html() {
		$schedule = $this->snappb_get_schedule();

		$days = $this->allowed_days;
		foreach ( $days as $d ) {
			if ( ! isset( $schedule[ $d ] ) || ! \is_array( $schedule[ $d ] ) ) {
				$schedule[ $d ] = array();
			}
		}
		?>
		<div class="snappbox-modal-content snappbox-date-content none-class" id="dates">
			<span class="snappbox-close">&times;</span>
			<h2><?php \esc_html_e( 'Select Day and Time', 'snappbox' ); ?></h2>

			<div class="selection-wrapper clearfix">
				<div class="snappbox-day-selector">
					<p><?php \esc_html_e( 'Select Day', 'snappbox' ); ?></p>
					<select id="snappbox-day" class="snappbox-day-selector">
						<?php
						$day_labels = array(
							'saturday'  => \esc_html_x( 'Saturday', 'weekday name', 'snappbox' ),
							'sunday'    => \esc_html_x( 'Sunday', 'weekday name', 'snappbox' ),
							'monday'    => \esc_html_x( 'Monday', 'weekday name', 'snappbox' ),
							'tuesday'   => \esc_html_x( 'Tuesday', 'weekday name', 'snappbox' ),
							'wednesday' => \esc_html_x( 'Wednesday', 'weekday name', 'snappbox' ),
							'thursday'  => \esc_html_x( 'Thursday', 'weekday name', 'snappbox' ),
							'friday'    => \esc_html_x( 'Friday', 'weekday name', 'snappbox' ),
						);

						foreach ( $days as $day ) :
							?>
							<option value="<?php echo \esc_attr( $day ); ?>">
								<?php
								echo isset( $day_labels[ $day ] )
									? \esc_html( $day_labels[ $day ] )
									: \esc_html( \ucfirst( $day ) );
								?>
							</option>
						<?php endforeach; ?>
					</select>
				</div>

				<div>
					<p><?php \esc_html_e( 'Add / Edit Time Slots', 'snappbox' ); ?></p>
					<div id="snappbox-time-slots">
						<?php foreach ( $schedule as $day => $slots ) : ?>
							<div class="snappbox-day-slots" data-day="<?php echo \esc_attr( $day ); ?>" style="display:none;">
								<?php foreach ( $slots as $slot ) : ?>
									<div class="time-slot">
										<input type="time" class="start" value="<?php echo \esc_attr( isset( $slot['start'] ) ? $slot['start'] : '' ); ?>" />
										<input type="time" class="end"   value="<?php echo \esc_attr( isset( $slot['end'] ) ? $slot['end'] : '' ); ?>" />
										<button type="button" class="remove-slot button" aria-label="<?php echo \esc_attr__( 'Remove slot', 'snappbox' ); ?>">
											<svg xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 48 48" width="16" height="16" aria-hidden="true"><path fill="#f44336" d="M44,24c0,11.045-8.955,20-20,20S4,35.045,4,24S12.955,4,24,4S44,12.955,44,24z"/><path fill="#fff" d="M29.656,15.516l2.828,2.828l-14.14,14.14l-2.828-2.828L29.656,15.516z"/><path fill="#fff" d="M32.484,29.656l-2.828,2.828l-14.14-14.14l2.828-2.828L32.484,29.656z"/></svg>
										</button>
									</div>
								<?php endforeach; ?>
							</div>
						<?php endforeach; ?>
					</div>

					<button type="button" id="add-time-slot" class="add-time button">
						<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 511.99" width="16" height="16" aria-hidden="true"><path fill="#ffffff" fill-rule="nonzero" d="M256 0c70.68 0 134.69 28.66 181.01 74.98C483.35 121.31 512 185.31 512 255.99c0 70.68-28.66 134.69-74.99 181.02-46.32 46.32-110.33 74.98-181.01 74.98-70.68 0-134.69-28.66-181.02-74.98C28.66 390.68 0 326.67 0 255.99S28.65 121.31 74.99 74.98C121.31 28.66 185.32 0 256 0zm116.73 236.15v39.69c0 9.39-7.72 17.12-17.11 17.12h-62.66v62.66c0 9.4-7.71 17.11-17.11 17.11h-39.7c-9.4 0-17.11-7.69-17.11-17.11v-62.66h-62.66c-9.39 0-17.11-7.7-17.11-17.12v-39.69c0-9.41 7.69-17.11 17.11-17.11h62.66v-62.67c0-9.41 7.7-17.11 17.11-17.11h39.7c9.41 0 17.11 7.71 17.11 17.11v62.67h62.66c9.42 0 17.11 7.76 17.11 17.11zm37.32-134.21c-39.41-39.41-93.89-63.8-154.05-63.8-60.16 0-114.64 24.39-154.05 63.8-39.42 39.42-63.81 93.89-63.81 154.05 0 60.16 24.39 114.64 63.8 154.06 39.42 39.41 93.9 63.8 154.06 63.8s114.64-24.39 154.05-63.8c39.42-39.42 63.81-93.9 63.81-154.06s-24.39-114.63-63.81-154.05z"/></svg>
						<?php \esc_html_e( 'Add Time Slot', 'snappbox' ); ?>
					</button>
				</div>
			</div>

			<div style="margin-top: 15px;text-align:left">
				<button type="button" id="save-snappbox-schedule" class="button button-primary colorful-button snappbox-btn">
					<?php \esc_html_e( 'Save', 'snappbox' ); ?>
				</button>
			</div>
		</div>
		<?php
	}

	public function snappb_save_schedule() {
		if ( ! \check_ajax_referer( 'snappbox_schedule_nonce', 'nonce', false ) ) {
			\wp_send_json_error( array( 'message' => \__( 'Invalid request.', 'snappbox' ) ), 400 );
		}

		if ( ! \current_user_can( 'manage_options' ) ) {
			\wp_send_json_error( array( 'message' => \__( 'Permission denied.', 'snappbox' ) ), 403 );
		}

		$post = array();
		if ( isset( $_POST ) && \is_array( $_POST ) ) {
			$post = \wp_unslash( $_POST );
		}

		$day = isset( $post['day'] ) ? \sanitize_text_field( $post['day'] ) : '';
		if ( ! \in_array( $day, $this->allowed_days, true ) ) {
			\wp_send_json_error( array( 'message' => \__( 'Invalid day value.', 'snappbox' ) ), 400 );
		}

		$raw_slots   = array();
		$slots_input = $post['slots'] ?? array();

		if ( \is_array( $slots_input ) ) {
			$raw_slots = \map_deep( $slots_input, 'sanitize_text_field' );
		} elseif ( \is_string( $slots_input ) ) {
			$decoded = \json_decode( $slots_input, true );
			if ( \json_last_error() === JSON_ERROR_NONE && \is_array( $decoded ) ) {
				$raw_slots = \map_deep( $decoded, 'sanitize_text_field' );
			} else {
				$raw_slots = array();
			}
		}

		if ( ! \is_array( $raw_slots ) ) {
			$raw_slots = array();
		}

		$slots_clean = array();
		foreach ( $raw_slots as $slot ) {
			if ( ! \is_array( $slot ) ) {
				continue;
			}

			$start = isset( $slot['start'] ) ? (string) $slot['start'] : '';
			$end   = isset( $slot['end'] ) ? (string) $slot['end'] : '';

			$start = $this->normalize_time_5( $start );
			$end   = $this->normalize_time_5( $end );

			if ( '' === $start && '' === $end ) {
				continue;
			}

			$slots_clean[] = array(
				'start' => $start,
				'end'   => $end,
			);
		}

		$saved_data = \get_option( 'snappbox_schedule', array() );
		if ( ! \is_array( $saved_data ) ) {
			$saved_data = array();
		}

		$saved_data[ $day ] = $slots_clean;
		\update_option( 'snappbox_schedule', $saved_data );

		\wp_send_json_success(
			array(
				'message' => \__( 'Schedule saved successfully!', 'snappbox' ),
				'data'    => $saved_data,
			)
		);
	}


	private function normalize_time_5( $time ) {
		$time = \trim( (string) $time );

		if ( '' === $time ) {
			return '';
		}

		if ( ! \preg_match( '/^\d{1,2}:\d{2}$/', $time ) ) {
			if ( \preg_match( '/^(\d{1,2}):(\d{2})/', $time, $m ) ) {
				$time = $m[1] . ':' . $m[2];
			} else {
				return '';
			}
		}

		list( $h, $m ) = \explode( ':', $time, 2 );
		$h             = (int) $h;
		$m             = (int) $m;

		if ( $h < 0 || $h > 23 || $m < 0 || $m > 59 ) {
			return '';
		}

		return \sprintf( '%02d:%02d', $h, $m );
	}

	public function snappb_get_schedule() {
		$opt = \get_option( 'snappbox_schedule', array() );
		return ( \is_array( $opt ) ) ? $opt : array();
	}

	public function snappb_get_day_slots( $day ) {
		$schedule = $this->snappb_get_schedule();
		return isset( $schedule[ $day ] ) && \is_array( $schedule[ $day ] ) ? $schedule[ $day ] : array();
	}
}
