...
class Vite {
     ...
     */
    public static function enqueue(string $entry, array $deps = [], bool $is_admin = true): void {
        if (self::is_dev()) {
    ...
    private static function enqueue_dev(string $entry, array $deps): void {
        // Enqueue Vite client
        \wp_enqueue_script('vite-client', self::SERVER_URL . '@vite/client', [], null, true);
        
        // Enqueue entry point
        \wp_enqueue_script('snappbox-' . $entry, self::SERVER_URL . 'src/' . $entry, ['vite-client'], null, true);
        
        // Fix for React HMR
        \add_action('wp_head', [self::class, 'vite_dev_header_scripts']);
        \add_action('admin_head', [self::class, 'vite_dev_header_scripts']);
    }
