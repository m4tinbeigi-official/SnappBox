...
class ScheduleModal {
    ...
    public function boot(): void {
        ...
        \add_action('wp_ajax_snappb_save_schedule', [$this, 'handle_save_schedule']);
    }
    ...
    public function handle_save_schedule(): void {
        ...
        if (!\in_array($day, $this->allowed_days, true)) {
            \wp_send_json_error(['message' => 'Invalid day'], 400);
        }

        $slots = \map_deep($_POST['slots'] ?? [], 'sanitize_text_field');
        $saved_data = \get_option('snappbox_schedule', []);
        $saved_data[$day] = $slots;
        
        \update_option('snappbox_schedule', $saved_data);
        \wp_send_json_success(['message' => 'Schedule saved!']);
    }
}
