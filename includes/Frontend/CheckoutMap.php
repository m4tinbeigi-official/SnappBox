...
/**
 * Checkout Map functionality for SnappBox.
 */
class CheckoutMap {
	...
	public function boot(): void {
		\add_action( 'wp_enqueue_scripts', array( $this, 'snappb_enqueue_map_scripts' ) );

		\add_action( 'woocommerce_before_checkout_billing_form', array( $this, 'snappb_display_osm_map' ) );
		\add_action( 'woocommerce_checkout_update_order_meta', array( $this, 'snappb_save_customer_location' ) );
		\add_action( 'woocommerce_checkout_process', array( $this, 'snappb_validate_customer_location' ) );
		\add_shortcode( 'snappbox_checkout_map', array( $this, 'snappb_display_osm_map' ) );

		\add_action( 'woocommerce_review_order_after_shipping', array( $this, 'snappb_render_snappbox_dates_row' ) );
		\add_action( 'wp_footer', array( $this, 'snappb_add_checkout_scripts' ) );
		\add_action( 'woocommerce_checkout_create_order', array( $this, 'snappb_save_order_meta' ), 10, 2 );
		\add_action( 'woocommerce_admin_order_data_after_shipping_address', array( $this, 'snappb_display_order_meta' ) );
		\add_action( 'woocommerce_order_details_after_order_table', array( $this, 'snappb_display_order_meta' ), 20, 1 );
	}
	...
	public function snappb_enqueue_map_scripts() {
		...
		\wp_register_style(
			'maplibre',
			$base_url . 'assets/css/leaflet.css',
			array(),
			'1.9.4'
		);
		\wp_register_script(
			'maplibre',
			$base_url . 'assets/js/leaflet.js',
			array(),
			'1.9.4',
			true
		);
		...
		\wp_register_style(
			'snappbox-checkout',
			$base_url . 'assets/css/snappbox-checkout.css',
			array( 'maplibre' ),
			'1.0.0'
		);
		...
		\wp_register_script(
			'snappbox-map',
			$base_url . 'assets/js/snappbox-map.js',
			array( 'maplibre' ),
			'1.0.0',
			true
		);
		...
		\wp_register_script(
			'snappbox-checkout',
			$base_url . 'assets/js/snappbox-checkout.js',
			array( 'jquery' ),
			'1.0.0',
			true
		);
	...
	public function snappb_display_osm_map() {
		...
		if ( empty( $settings['snappbox_latitude'] ) || empty( $settings['snappbox_longitude'] ) ) {
			return;
		}
	...
	public function snappb_save_customer_location( $order_id ) {
		...
		if ( isset( $_POST['customer_latitude'], $_POST['customer_longitude'] ) ) {
			...
			if ( $order ) {
				...
				$order->update_meta_data( 'customer_city', $customerCity );
				$order->update_meta_data( 'customer_state', $customerState );
				$order->update_meta_data( 'customer_country', $customerCountry );
				$order->update_meta_data( 'customer_postcode', $customerPost );
				$order->update_meta_data( 'customer_address', $customerAddress );
				$order->save();
			}
		}
	}
	...
	public function snappb_render_snappbox_dates_row() {
		$schedule = \get_option( 'snappbox_schedule', array() );
	...
	public function snappb_add_checkout_scripts() {
		...
		\wp_localize_script(
			...
			array(
				...
				'rtlPlugin'      => \trailingslashit( $base_url ) . 'assets/js/mapbox-gl-rtl-text.js',
				'reverseUrl'     => 'https://api.teh-1.snappmaps.ir/reverse/v1',
				'reverseHeaders' => array(
					'Accept'        => 'application/json',
					'X-Smapp-Key'   => \defined( 'SNAPPBOX_SMAPP_KEY' ) ? \SNAPPBOX_SMAPP_KEY : '',
					'Authorization' => \defined( 'SNAPPBOX_MAP_TOKEN' ) ? \SNAPPBOX_MAP_TOKEN : '',
				),
				'nominatimUrl'   => 'https://api.teh-1.snappmaps.ir/reverse/v1',
			)
		);

		$raw_schedule  = \get_option( 'snappbox_schedule', array() );
		$weekly        = ( \is_array( $raw_schedule ) && ! empty( $raw_schedule ) ) ? $this->snappb_sb_normalize_schedule_to_w( $raw_schedule ) : array();
		$candidates    = array();
		$times_by_date = array();
